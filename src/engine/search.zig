...
pub const Searcher = struct {
    ...
    pub fn negamax(self: *Searcher, board: *brd.Board, color: brd.Color, depth_: usize, alpha_: i32, beta_: i32, is_null: bool, comptime node_type: NodeType, cutnode: bool) i32 {
        ...
        for (0..move_count) |i| {
            ...
            if (!is_root and depth >= 8 and tt_hit and entry.?.flag != tt.EstimationType.Over and !eval.almostMate(tt_eval) and move.matchesTTKey(hash_move) and entry.?.depth >= depth - 3 and move_list.len >= 2) {
                const margin: i32 = @as(i32, @intCast(depth)) * 4;
